...
export class BoardsApiHelper {
    ...
    createBoard() {    
        ...
        return cy.postRequest(BoardsEndpointConstants.POST_BOARD, requestBody)
            .as('createBoard')   
    }
    ...
    updateBoard(boardId) {    
        ...
        return cy.putRequest(endpoint, requestBody)
            .as('updateBoard')   
    }
    ...
    deleteBoardById(boardId) {
        ...
        return cy.deleteRequest(endpoint)
            .as('deleteBoardById')   
    }

    deleteAllBoards() {
        organizationsApiHelper.getBoardsInOrganization().then(
            (allBoardsInOrganization) => {
                for (let i = 0; i < allBoardsInOrganization.body.length; i++) {
                    this.deleteBoardById(
                        allBoardsInOrganization.body[i].id
                    )
                }
            }
        )
